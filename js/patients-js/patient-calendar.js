...
import { timeSpanToMinutes } from './patient-utils.js';
...
/**
 * Carga fechas y horarios disponibles para un doctor
 */
export async function loadAvailableDatesAndTimes(doctorId) {
    try {
        ...
        const appointmentsResponse = await ApiScheduling.get(
            `v1/Appointments?doctorId=${doctorId}&startTime=${now.toISOString()}&endTime=${fourWeeksLater.toISOString()}`
        );
 ...
 */
export async function loadAvailableTimes(doctorId, selectedDate) {
    try {
        ...
        const appointmentsResponse = await ApiScheduling.get(
            `v1/Appointments?doctorId=${doctorId}&startTime=${startOfDay.toISOString()}&endTime=${endOfDay.toISOString()}`
        );
        const appointments = Array.isArray(appointmentsResponse) 
            ? appointmentsResponse 
            : (appointmentsResponse?.value || appointmentsResponse || []);
